...
int main( int argc, char * argv[] )
{
  ...
  // TEST 1
  mavalloc_init( MAX_SIZE, FIRST_FIT );
  for(int i = 0; i < TEST_ITERATION; i++)
  {
    //starts the timer
    start = clock();
    ...
    time[i] = timeTaken;
    printf("Time taken for benchmark2.c Test 1 Iteration %d: %0.2f microseconds\n",i, timeTaken);
  }
  export(time);
  mavalloc_destroy();
  ...
  for(i = 0; i < TEST_ITERATION; i++)
  {
    ...
    double timeTaken = (double)((end-start)*1000000/CLOCKS_PER_SEC);
    time[i] = timeTaken;
    printf("Time taken for benchmark2.c Test 2 Iteration %d: %0.2f microseconds\n",i, timeTaken);
  }
  export(time);
